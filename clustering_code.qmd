...
write_csv(df_pca,paste("pcaResults_",myPath,".csv",sep=""))
```
...
```{r}
fviz_nbclust(pca_res$x,kmeans,method="wss",k.max=10)
```
...
```{r}
km <- kmeans(pca_res$x,centers=10,nstart=3,iter.max=100)
```
...
write_csv(clustered_files,paste("clustered_",myPath,".csv",sep=""))
```

```{r}
clustered_files |> filter(cluster==2)
```
